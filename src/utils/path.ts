...
export const path: Path = {
     ...
     */
    toPosix(path: string) { return path.replaceAll('\\', '/'); },
    /**
     * Checks if the path is a URL
     * @param path - The path to check
     */
    isUrl(path: string) { return path.startsWith('http'); },
    /**
    ...
    toAbsolute(url: string, customBaseUrl?: string, customRootUrl?: string)
    {
        ...
        const baseUrl = removeUrlParams(this.toPosix(customBaseUrl ?? os.getenv('PWD') ?? '/'));
