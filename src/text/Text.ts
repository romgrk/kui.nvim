...
import type { ICanvas, Renderer } from 'src/core';
import type { CanvasRenderingContext2D } from 'src/canvas-renderer/CanvasContextSystem'
...
export class Text extends Sprite
{
    ...
    /**
     * If {_link PIXI.Text.defaultAutoResolution} is false, this will be the default resolution of the text.
     * If not set it will default to {_link PIXI.settings.RESOLUTION}.
     * @example
     * Text.defaultResolution = 2;
     * Text.defaultAutoResolution = false;
     *
     ...
     */
    constructor(text?: string | number, style?: Partial<ITextStyle> | TextStyle)
    {
        const canvas = settings.ADAPTER.createCanvas(3, 3);
        ...
        this._text = null as any;
        this._style = null as any;
        this._styleListener = null as any;
        this._font = '';
    ...
    public updateText(respectDirty: boolean): void
    {
        ...
        this.canvas.setDimensions(
            Math.ceil(Math.ceil((Math.max(1, width) + (style.padding * 2))) * this._resolution),
            Math.ceil(Math.ceil((Math.max(1, height) + (style.padding * 2))) * this._resolution),
        )
        ...
        for (let i = 0; i < passesCount; ++i)
        {
            ...
            for (let i = 0; i < lines.length; i++)
            {
                ...
                if (style.stroke !== '' && style.stroke !== 0 && style.strokeThickness !== 0)
                {
    ...
    ): string
    {
        ...
        //       the setter converts to string. See this thread for more details:
        //       https://github.com/microsoft/TypeScript/issues/2521
        const fillStyle: string | string[] = style.fill as any;
        ...
        throw new Error('Gradient style unimplemented')
     ...
     *
     * If setting the `style` can also be partial {_link PIXI.ITextStyle}.
     */
    get style(): TextStyle
    {
        return this._style;
