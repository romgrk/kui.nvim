...
export function earcut(data: number[], holeIndices?: number[] | null, dim?: number) {

    dim = dim || 2;

    let hasHoles = holeIndices && holeIndices.length
    let outerLen = (holeIndices && holeIndices.length) ? holeIndices[0] * dim : data.length
    let outerNode = linkedList(data, 0, outerLen, dim, true)
    let triangles = [] as number[]
